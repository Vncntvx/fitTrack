...
class _HistoryTabState extends ConsumerState<HistoryTab> {
      ...
      children: [
        ...
        Padding(
          ...
          child: SingleSelectChipGroup<String>(
            options: [
              const FilterOption(value: 'all', label: '全部'),
              FilterOption(
                value: 'strength',
                label: '力量',
        ...
        Expanded(
          child: AsyncValueWidget(
            ...
            dataBuilder: (sessions) => _buildSessionList(context, ref, sessions),
          ),
        ),
      ],
    );
  ...
  Widget _buildSessionList(
    ...
    var filtered = sessions.where((session) {
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        if (!typeLabel.toLowerCase().contains(_searchQuery.toLowerCase()) &&
            !(session.note?.toLowerCase().contains(_searchQuery.toLowerCase()) ?? false)) {
          return false;
        }
      }
      return true;
    ...
    final sessionWithDetails = filtered
        .map((s) => SessionWithDetails(session: s))
        .toList();
