...
class _SwimmingSessionPageState extends ConsumerState<SwimmingSessionPage> {
  ...
  /// 泳姿选择器 - 使用 SegmentedButton 避免 3+1 换行
  Widget _buildStrokeSelector(SwimmingSessionState state) {
    final strokeOptions = _strokes
    ...
    return Column(
      ...
      children: [
        ...
        SingleSelectSegmented<String>(
          ...
          selected: state.primaryStroke,
          onChanged: (value) {
            ref.read(swimmingSessionProvider.notifier).setStroke(value);
          },
          enabled: !_isReadOnly,
        ),
      ],
        ...
        if (!_isReadOnly)
          QuickSelectRow<int>(
            options: [500, 1000, 1500, 2000]
                .map((m) => SelectOption(value: m, label: '${m}m'))
                .toList(),
            selected: state.distanceMeters.toInt(),
            onChanged: (meters) {
  ...
  /// 训练类型选择 - 使用 SegmentedButton (4 个短选项)
  Widget _buildTrainingTypeSelector(SwimmingSessionState state) {
    final typeOptions = _trainingTypes
    ...
    return Column(
      ...
      children: [
        ...
        SingleSelectSegmented<String>(
          options: typeOptions,
          selected: state.trainingType ?? 'endurance',
          onChanged: (value) {
            ref.read(swimmingSessionProvider.notifier).setTrainingType(value);
          },
          enabled: !_isReadOnly,
        ),
      ],
  ...
  /// 装备选择 - FilterChip 可以保持 Wrap，因为多选场景且数量可变
