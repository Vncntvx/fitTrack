...
Always use Context7 MCP tools before making any library/API decisions for this Flutter project.

Required behavior:

1. Use `resolve-library-id` and `query-docs` to fetch the latest docs for any Flutter / Material / package API you touch.
2. Prefer official Flutter and Material documentation when making UI, theming, adaptive layout, navigation, accessibility, and component decisions.
3. Never assume an API from memory if Context7 can verify it.
4. For UI refactors, first audit the current implementation, then propose a plan, then implement.
5. Keep business logic unchanged unless explicitly requested.
6. Prefer app-level theming over per-widget styling.
7. Avoid hard-coded colors, inconsistent radii, random spacing, and one-off text styles.
8. For responsive/adaptive work, optimize for phones, tablets, and large screens using layout constraints/window size rather than device marketing names.
9. Before finalizing, run static analysis and fix UI-related warnings where reasonable.
10. Output:
